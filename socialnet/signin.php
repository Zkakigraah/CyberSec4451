...
// VULNERABLE VERSION (Replace lines 19-38 in signin.php)
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $username = $_POST['username'];
    $password = $_POST['password'];

    // VULNERABLE: Direct string concatenation
    $sql = "SELECT id, fullname, password FROM account WHERE username = '$username'";
    $result = $conn->query($sql);

    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        // The attacker will use a UNION query to supply their own hash here
